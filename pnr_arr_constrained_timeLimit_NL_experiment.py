...
for instance in [0]:#range(10):
    for (multiple, timeLimit) in [(1,7200 * 2)]:
        for (numHubsB,numAlternativesB,numOriginsB) in [(60,120,400)]:#[(6,30,100),(12,30,100),(18,30,100),(24,30,100)]:
            for rep in [3]:                
                for lambdaNL in [0.5,0.25,0.75]:
                    ...
                    bestSolution.to_csv(r'arr_constrained_pnr_origin%s_I%s_J%s_p%s_inst%s_rep%s_lambda%s.csv'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs,instance,rep,int(lambdaNL*10)), index = False)#Check
