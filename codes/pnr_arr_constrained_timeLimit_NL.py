...
bestSolution.to_csv(r'arr_constrained_pnr_origin%s_I%s_J%s_p%s_inst%s.csv'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs,instance), index = False)#Check
...
while elapseTime < timeLimit:
    ...
    if same == True:
        ...
        if bestDemand < totalDemand:
            ...
            bestSolution.to_csv(r'arr_constrained_pnr_origin%s_I%s_J%s_p%s_inst%s_lambda(%s).csv'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs,instance,lambdaNL), index = False)#Check
